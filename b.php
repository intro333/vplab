...
//    if ($link !== '/shop.php?op=price&vid=200') {
    if ($link == '/shop.php?op=price&vid=45') {
        ...
        if (!$wsql2->query('SELECT text FROM table')) {
            print "Query error: " . $wsql2->error;
            exit;
        }
        $links2 = array();
        foreach($wsql2->fetch_array() as $row2){
            $links2[] = $row2['text'];
        }
        var_dump($links2);
        exit;
        ...
        $html2->load($ItemLinks);
        //Проходимся по всем table body
        $html2TableBody = $html2->find('div[data-sku=U3036]');
        var_dump($html2TableBody);
        exit;
        foreach($html2TableBody as $keyObg => $obj) {

            $html = new simple_html_dom();
            $html->load($obj);

            //Наименование
            $arrayForDescriptionElements = array();
